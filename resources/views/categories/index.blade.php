                ...
                @forelse($categories as $category)
                    @php
                        ...
                        if (!empty($category->image_url)) {
                            ...
                            $img = asset($category->image_path);
                        } elseif (!empty($category->image_path) && \Illuminate\Support\Facades\Storage::disk('public')->exists($category->image_path)) {
                            $img = \Illuminate\Support\Facades\Storage::url($category->image_path);
                        } else {
                            $img = asset('images/produit.png');
                        }

                        $title = $category->nom ?? $category->name ?? 'Catégorie';
                    ...
                    <article class="group relative bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition transform hover:-translate-y-1"
                             ...
                             data-desc="{{ $dataDesc }}">
                        <a href="{{ route('categories.show', $category) }}" class="absolute inset-0 z-10" aria-label="Ouvrir la catégorie {{ $title }}"></a>
                        ...
                        <div class="p-4 sm:p-5">
                            <div class="flex items-start justify-between gap-3">
                                ...
                                @if(method_exists($category, 'puzzles'))
                                    <div class="flex-shrink-0 ml-2 hidden sm:block">
                                        <span class="inline-flex items-center px-3 py-1 rounded-full bg-amber-100 text-amber-800 text-xs font-medium">
                                            {{ $category->puzzles()->count() }} puzzles
                                        </span>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="mt-4 flex items-center justify-between gap-3">
                                ...
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('categories.show', $category) }}" class="text-amber-600 text-sm font-semibold hover:underline">Voir</a>
